...
CMD set -xe; echo "Starting on PORT=${PORT:-8000}"; python manage.py migrate --noinput; gunicorn back.wsgi:application --bind "0.0.0.0:${PORT:-8000}" --bind "[::]:${PORT:-8000}" --workers 1 --timeout 120 --access-logfile - --error-logfile - --log-level debug --capture-output
